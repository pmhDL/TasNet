...
class FusionTrainer(object):
    def __init__(self, args):
        ...
        meta_base_dir = osp.join(log_base_dir, 'fusion')
        ...
        self.val_loader = DataLoader(dataset=self.valset, batch_sampler=self.val_sampler, num_workers=8,
                                     pin_memory=True)
        #build model
        self.model = TasLearner(self.args, mode='fusion')
        # Set optimizer
        self.optimizer = torch.optim.Adam([
            #{'params': self.model.word_learner.parameters(), 'lr': self.args.meta_lr2},
            {'params': self.model.feat_learner.parameters(), 'lr': self.args.vlr}], weight_decay=1e-4)
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.max_epoch + 1):
            ...
            for i, batch in enumerate(tqdm_gen, 1):
                ...
                lg_a = self.model((data_shot, label_shot, emb_s, data_query))
